...
# --------------------------------------
def decimalToBinary(decimalInitial, targetBinaryPrecision = beta):
...
def dyadicDecoder(decimalInitial, k):
    return (2 ** (k * beta) * decimalInitial) % 1

def logisticDecoder(decimalInitial, k):
    return float(Sin(2 ** (k * beta) * Asin(Sqrt(decimalInitial))) ** 2)
...
def findInitialCondition(trainData):
    ...
    assert beta * len(trainData) == necessaryPrecision
